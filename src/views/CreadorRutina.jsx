...
export default function CreadorRutina({ cambiarVista }) {
  ...
  // Agregamos 'archivoAudio: null' al estado base
  ...
  // --- ESTADOS PARA MODALIDADES DINÁMICAS ---
  ...
  useEffect(() => {
    ...
    // Carga de Modalidades (Categorías Dinámicas)
  ...
  // ==========================================
  // MANEJO DE CATEGORÍAS (MODALIDADES)
  // ==========================================
  ...
  // ==========================================
  // MANEJO DE AUDIO CUSTOM
  // ==========================================
  const manejarSubidaAudio = (e) => {
    const file = e.target.files[0];
    if (file) {
      const fileUrl = URL.createObjectURL(file);
      // Le quitamos la extensión .mp3 al nombre por defecto
  ...
  const guardarEjercicioPersonalizado = (e) => {
    ...
    if (ejercicioAEditar.id) {
      nuevaLista = ejerciciosPersonalizados.map((ej) =>
        ej.id === ejercicioAEditar.id ? { ...ejercicioAEditar, isCustom: true } : ej
      );
    } else {
      nuevaLista = [
  ...
  const agregarEtapa = () => {
    ...
    // Reseteamos incluyendo los audios
    setEtapaEnConstruccion({
      id: "temp",
      modalidad: "Fuerza",
      duracionMinutos: 10,
  ...
  // ==========================================
  // GESTIÓN DE EJERCICIOS (Con Autocompletado Custom)
  // ==========================================
  ...
  const agregarEjercicioSimple = (e) => {
    ...
    setEtapaEnConstruccion({
      ...
      listaEjercicios: [
        ...
        { tipo: "simple", ejercicio: { ...nuevoEjercicio, cantidad: repeticiones } },
      ],
    });
  ...
  const confirmarBloqueCombinado = () => {
    ...
    setEtapaEnConstruccion({
      ...
      listaEjercicios: [
        ...etapaEnConstruccion.listaEjercicios,
        { tipo: "combinado", ejercicios: bloqueCombinadoTemp, cantidad: repeticionesBloque },
      ],
    });
    ...
    <div className="min-h-screen w-screen bg-gray-900 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-gray-900 via-gray-900 to-black overflow-x-hidden p-4 md:p-8">
      ...
      <header className="flex justify-between items-center mb-8 border-b border-white/10 pb-6">
        <div className="flex items-center gap-6 w-full max-w-2xl">
          <button onClick={volverAtras} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/5">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-md relative">
            ...
            <div className="flex gap-4 mb-4 mt-2">
              <div className="flex-1">
                ...
                <select
                  ...
                  onChange={manejarCambioModalidad}
              ...
              <div className="w-1/3">
                ...
                <input
                  ...
                  onChange={(e) => setEtapaEnConstruccion({ ...etapaEnConstruccion, duracionMinutos: parseInt(e.target.value) || 0 })}
                  className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-cyan-500"
                />
              </div>
            </div>

            {/* SECCIÓN DE AUDIO / MÚSICA ACTUALIZADA */}
            <div className="mb-6 bg-black/20 p-4 rounded-xl border border-white/5">
              ...
              <div className="flex flex-col gap-3">
                {/* Opciones Internas (Buzzer, Pitidos) */}
                <select 
                  value={etapaEnConstruccion.tieneAudioPersonalizado ? "Personalizado" : etapaEnConstruccion.audioPista} 
                  onChange={(e) => {
                    if (e.target.value !== "Personalizado") {
                      setEtapaEnConstruccion({...etapaEnConstruccion, audioPista: e.target.value, tieneAudioPersonalizado: false, archivoAudio: null});
                    }
                  }} 
                  className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-cyan-500"
                >
                ...
                {/* Zona de Subida y Renombrado de MP3 */}
                <div className="relative">
                  {!etapaEnConstruccion.tieneAudioPersonalizado && (
                    <input 
                      type="file" 
                      accept="audio/*" 
                      onChange={manejarSubidaAudio}
                      className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                      title="Subir pista de TabataSongs / MP3"
                    />
                  )}
                  <div className={`flex flex-col gap-2 p-3 rounded-xl border transition-all ${etapaEnConstruccion.tieneAudioPersonalizado ? "bg-cyan-500/10 border-cyan-500/50" : "bg-white/5 border-white/10 hover:border-cyan-500/30"}`}>
                    ...
                    {!etapaEnConstruccion.tieneAudioPersonalizado ? (
                      ...
                      <div className="flex items-center gap-3 relative z-20">
                        ...
                        {/* INPUT PARA MODIFICAR EL NOMBRE DEL AUDIO */}
                        <input 
                          type="text" 
                          value={etapaEnConstruccion.audioPista}
                          onChange={(e) => setEtapaEnConstruccion({...etapaEnConstruccion, audioPista: e.target.value})}
                          className="flex-1 bg-black/40 border border-cyan-500/30 rounded-lg p-2 text-cyan-300 font-bold focus:outline-none focus:border-cyan-400 text-sm"
                          placeholder="Nombre de la pista..."
                        />
                        {/* Botón para eliminar el audio subido */}
                        <button 
                          type="button" 
                          onClick={() => setEtapaEnConstruccion({...etapaEnConstruccion, audioPista: "Sin Audio", tieneAudioPersonalizado: false, archivoAudio: null})} 
            ...
            <div className="border-t border-white/10 pt-6">
              ...
              {modoIngreso === "simple" ? (
                <form onSubmit={agregarEjercicioSimple} className="flex gap-2 mb-4 relative overflow-visible">
                  <button type="button" onClick={() => setModalBibliotecaAbierto(true)} className="p-3 bg-white/5 hover:bg-cyan-500/20 border border-white/10 rounded-xl text-cyan-400 transition-all shrink-0">
                    ...
                    {mostrarSugerencias && nuevoEjercicio.nombre.length > 0 && sugerenciasFiltradas.length > 0 && (
                      <ul className="absolute z-50 w-full bg-gray-800 border border-cyan-500/30 rounded-xl mt-1 max-h-48 overflow-y-auto shadow-xl">
                        {sugerenciasFiltradas.map((ej) => (
                          ...
                          >
                            <span>{ej.name}</span>
                            <span className="text-xs text-cyan-300 font-mono">{ej.equipment}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                  ...
                  />
                  <button type="submit" className="px-4 shrink-0 bg-white/10 hover:bg-white/20 rounded-xl font-bold text-white transition-all">+</button>
                </form>
              ) : (
                ...
                <div className="bg-fuchsia-500/5 border border-fuchsia-500/20 rounded-2xl p-4 mb-6 relative overflow-visible">
                  ...
                  <form onSubmit={agregarNombreAlBloqueCombinado} className="flex gap-2 mb-4">
                    <button type="button" onClick={() => setModalBibliotecaAbierto(true)} className="p-3 shrink-0 bg-white/5 hover:bg-fuchsia-500/20 border border-white/10 rounded-xl text-fuchsia-400 transition-all">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                    </button>
                      ...
                      {mostrarSugerencias && nuevoEjercicio.nombre.length > 0 && sugerenciasFiltradas.length > 0 && (
                        <ul className="absolute z-50 w-full bg-gray-800 border border-fuchsia-500/30 rounded-xl mt-1 max-h-48 overflow-y-auto shadow-xl">
                          {sugerenciasFiltradas.map((ej) => (
                            ...
                            >
                              <span>{ej.name}</span>
                              <span className="text-xs text-fuchsia-300 font-mono">{ej.equipment}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <button type="submit" className="px-4 shrink-0 bg-fuchsia-500/20 hover:bg-fuchsia-500/40 text-fuchsia-300 rounded-xl font-bold transition-all">Sumar</button>
                  </form>
                  ...
                  <div className="border-t border-fuchsia-500/20 pt-4">
                    <span className="text-xs font-bold text-fuchsia-400 uppercase tracking-wider mb-2 block">Repeticiones Totales del Bloque</span>
                  ...
                  {etapaEnConstruccion.listaEjercicios.map((bloque, index) => (
                    <div key={index} className="flex justify-between items-center text-sm p-3 bg-white/5 border border-white/5 rounded-xl">
                      <div className="flex flex-col">
                        {bloque.tipo === "simple" && (
                          <span>
                            {bloque.ejercicio.nombre} <span className="text-cyan-400 font-mono ml-2">{bloque.ejercicio.cantidad}</span>
                          </span>
                        )}
                          ...
                          <span className="text-gray-200">
                            {bloque.ejercicios.map((ej, i) => (
                              <React.Fragment key={i}>
                                {i > 0 && <span className="text-fuchsia-400 font-bold mx-1">+</span>}
                                {ej}
                              </React.Fragment>
                            ))}
                            <span className="text-fuchsia-400 font-mono ml-2">({bloque.cantidad})</span>
                          </span>
                        )}
                      </div>
                      <button onClick={() => eliminarBloqueDeEtapa(index)} className="text-gray-500 hover:text-rose-500">
                ...
                {etapas.map((etapa) => (
                  <div key={etapa.id} className="scale-90 origin-top relative group">
                    ...
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] rounded-2xl opacity-0 group-hover:opacity-100 flex items-center justify-center gap-4 transition-all">
                      <button onClick={() => editarEtapa(etapa.id)} className="p-4 bg-cyan-500 text-black rounded-full shadow-[0_0_20px_rgba(34,211,238,0.4)] hover:scale-110">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                      </button>
                      <button onClick={() => eliminarEtapa(etapa.id)} className="p-4 bg-rose-500 text-white rounded-full shadow-[0_0_20px_rgba(244,63,94,0.4)] hover:scale-110">
            ...
            {creandoEjercicio ? (
              <form onSubmit={guardarEjercicioPersonalizado} className="bg-black/40 border border-white/10 rounded-2xl p-6 mb-6">
                <h3 className="text-lg font-bold text-white mb-4">
                  {ejercicioAEditar.id ? "Editar Ejercicio" : "Crear Nuevo Ejercicio"}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEjercicioAEditar({ ...ejercicioAEditar, name: e.target.value })}
                      className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-fuchsia-500"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEjercicioAEditar({ ...ejercicioAEditar, category: e.target.value })}
                      className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-fuchsia-500"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEjercicioAEditar({ ...ejercicioAEditar, equipment: e.target.value })}
                      className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-fuchsia-500"
            ...
            {!creandoEjercicio && (
              <div className="flex-1 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pr-2 pb-4">
                {ejerciciosPersonalizados
                  ...
                  .filter((ej) =>
                    (ej.name && ej.name.toLowerCase().includes(busquedaEjercicio.toLowerCase())) ||
                    (ej.equipment && ej.equipment.toLowerCase().includes(busquedaEjercicio.toLowerCase()))
                  )
                  .map((ej, i) => (
                    <div key={i} className={`flex flex-col text-left p-4 rounded-2xl transition-all group relative border bg-fuchsia-500/5 border-fuchsia-500/30`}>
                      ...
                      <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => abrirEdicionEjercicio(e, ej)} className="p-1 text-gray-400 hover:text-cyan-400" title="Editar">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button onClick={(e) => eliminarEjercicioPersonalizado(e, ej.id)} className="p-1 text-gray-400 hover:text-rose-500" title="Eliminar">
